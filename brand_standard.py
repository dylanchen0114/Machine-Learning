...
class BrandSeparator:
    ...
    def get_frame(self):
        ...
        mapping_frame['brand'] = mapping_frame.index
...
def main():
    """
    AllData = pd.DataFrame({'brand':
                                ['美滋滋', '欧莱雅', '欧莱雅中国/LOREAL', 'LOREAL', '欧莱雅中国/bbb', '欧莱雅/loreal', '欧雅/loreal', '欧莱雅/lrl', '/loreal',
                                 '欧莱雅/aaa',
                                 '兰蔻/lancome', '悦诗风吟/inisfree', '悦诗风吟/inis', 'ccc']})
    """
    AllData = pd.read_csv('~/Documents/brand_stadard/jumei_brand_csv')
    brand_list = BrandSeparator(AllData['brand'])
    ...
    AllData = AllData.merge(brand_list_frame[['brand', 'cln_chn_name', 'cln_en_name']], how='left', on='brand')
    ...
    AllData.rename(columns={'standard_brand_x': 'standard_chn_name', 'standard_brand_y': 'standard_en_name'},
                   inplace=True)
